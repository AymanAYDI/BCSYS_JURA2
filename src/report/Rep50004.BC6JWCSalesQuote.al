...
report 50004 "BC6 JWC Sales Quote"
{
    DefaultLayout = RDLC;
    ...
    dataset
    {
        ...
        dataitem(Header; "Sales Header")
        {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                trigger OnAfterGetRecord()
                begin
                    DocumentTitle := StrSubstNo('%1 %2', Header."Document Type", Header."No.");
                    OutputNo += 1;
                    if Number > 1 then
            ...
            var
                ToDo: Record "To-do";
                Text001: Label 'Do you want to create a follow-up to-do?';
            begin
                MarkedOnly := true;
                Commit();
                CurrReport.Language := GlobalLanguage;
                if CurrentClientType <> ClientType::Web then
                    if FindSet() and ToDo.WritePermission then
    ...
    requestpage
    {
        ...
        var
            SegManagement: Codeunit SegManagement;
        begin
            LogInteractionG := (SegManagement.FindInteractionTemplateCode("Interaction Log Entry Document Type"::"Sales Qte.") <> '');
